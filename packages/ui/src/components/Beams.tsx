...
const mixColors = (backgroundColor: string, lightColor: string, amount: number) => {
  return new THREE.Color(backgroundColor).lerp(new THREE.Color(lightColor), amount);
};
...
const Beams: FC<BeamsProps> = ({
  ...
  const [resolvedThemeColors, setResolvedThemeColors] = useState(getActiveThemeColors);
  const resolvedLightColor = lightColor ?? resolvedThemeColors.primary;
  const resolvedBackgroundColor = backgroundColor ?? resolvedThemeColors.background;

  useEffect(() => {
    const syncThemeColors = () => {
      setResolvedThemeColors(getActiveThemeColors());
    };
  ...
  const surfaceColor = useMemo(
    () => mixColors(resolvedBackgroundColor, resolvedLightColor, 0.18),
    [resolvedBackgroundColor, resolvedLightColor]
  );
  const emissiveColor = useMemo(() => new THREE.Color(resolvedLightColor), [resolvedLightColor]);
  ...
  vec3 getNormal(vec3 pos) {
    ...
    [emissiveColor, noiseIntensity, scale, speed, surfaceColor]
  );
    ...
    <CanvasWrapper>
      <group rotation={[0, 0, THREE.MathUtils.degToRad(rotation)]}>
        <PlaneNoise
          ...
          height={beamHeight}
        />
        <DirLight color={resolvedLightColor} intensity={1.35} position={[0, 3, 10]} />
      </group>
      <ambientLight color={resolvedLightColor} intensity={0.12} />
...
>(({ beamGapPx, material, width, count, height }, ref) => {
  ...
  const spacing = useMemo(() => {
    ...
    return (viewport.width / size.width) * beamGapPx;
  }, [beamGapPx, size.width, viewport.width]);
  const geometry = useMemo(
    () => createStackedPlanesBufferGeometry(count, width, height, spacing, 100),
    [count, height, spacing, width]
  );
  useFrame((_, delta) => {
